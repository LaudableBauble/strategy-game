...
namespace StrategyGameHelper
{
    public static class Helper
    {
        ...
        public static bool IsValidNeighbors(int tileIndex1, int tileIndex2, Direction dir, LockBitmap img, TileImage ti)
        {
            //Get the tiles' position.
            tileIndex1 *= ti.TileSize;
            tileIndex2 *= ti.TileSize;
            int startX1 = tileIndex1 % img.Width;
            int startY1 = (tileIndex1 - startX1) / img.Width;
            int startX2 = tileIndex2 % img.Width;
            int startY2 = (tileIndex2 - startX2) / img.Width;

            //Make sure the image is locked.
            if (!img.IsLocked) { img.LockBits(); }

            int pixelValueThreshold = 50;
            int failCountThreshold = 10;
            var aboveThreshold = 0;

            //Check border pixels.
            for (int i = 0; i < ti.TileSize; i++)
            {
                ...
                switch (dir)
                {
                    case Direction.East:
                        {
                            c1 = img.GetPixel(startX1 + ti.TileSize - 1, startY1 + i);
                            c2 = img.GetPixel(startX2, startY2 + i);
                            break;
                        }
                    case Direction.West:
                        {
                            c1 = img.GetPixel(startX1, startY1 + i);
                            c2 = img.GetPixel(startX2 + ti.TileSize - 1, startY2 + i);
                            break;
                        }
                    case Direction.North:
                        {
                            c1 = img.GetPixel(startX1 + i, startY1);
                            c2 = img.GetPixel(startX2 + i, startY2 + ti.TileSize - 1);
                            break;
                        }
                    case Direction.South:
                        {
                            c1 = img.GetPixel(startX1 + i, startY1 + ti.TileSize - 1);
                            c2 = img.GetPixel(startX2 + i, startY2);
                            break;
                        }
                }

                if (Math.Abs(c1.R - c2.R) > pixelValueThreshold) { aboveThreshold++; }
                if (Math.Abs(c1.G - c2.G) > pixelValueThreshold) { aboveThreshold++; }
                if (Math.Abs(c1.B - c2.B) > pixelValueThreshold) { aboveThreshold++; }

                if (aboveThreshold > failCountThreshold) { return false; }
            }
